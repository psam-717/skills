...
5. **If there are uncommitted code changes**, commit them one file per commit:
   Run `git status --short` to check for modified/new/deleted files.
   If non-empty, load the `granular-commits-pr` skill with
   `skill_view(name="granular-commits-pr")` and follow its workflow to commit
   each modified/new/deleted file individually with conventional commit messages.
   Do NOT batch all changes into a single "release: prepare" commit — granular
   commits preserve history and make the changelog clear.
   If `git status --short` is already empty, skip this step entirely.

6. **Verify** `git status --short` shows no remaining changes (excluding ignored files) before moving to Phase 2.
...
> **Note:** The version bump is committed later in **Phase 6.5 (Post-Release)**
> alongside the tag and push. This keeps the release commit atomic: the tag
> points to the exact commit where the version changed.
...
## Phase 6.5 — Post-Release: Bump Commit, Tag, Push
...
1. **Commit the version bump and push everything:**
   The `pyproject.toml` version was updated in Phase 2 but left uncommitted.
   Now commit it so the tag points to the exact version-change commit:
   ```bash
